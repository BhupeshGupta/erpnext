...
def execute(filters=None):
	...
	rs_data = get_display_entries(filters)
...
def get_columns():
	return [_("Posting Date") + ":Date:100", _("Journal Voucher") + ":Link/Journal Voucher:220",
			...
			_("Against Account") + ":Link/Account:200", _("Reference") + "::100", _("Ref Date") + ":Date:110",
			_("Clearance Date") + ":Date:110"
	]
...
def get_display_entries(filters):
	...
	uncleared_entries = frappe.db.sql("""select
			jv.posting_date, jv.name, jvd.debit, jvd.credit,
			jvd.against_account, jv.cheque_no, jv.cheque_date, jv.clearance_date
		from
			`tabJournal Voucher Detail` jvd, `tabJournal Voucher` jv
